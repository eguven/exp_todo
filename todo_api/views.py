...
def handle_get(request, pk=None):
    '''handle GET request'''
    if pk is None:
        items = [item.to_json() for item in request.user.todoitem_set.all()]
        return json_response(items)
    try:
        item = request.user.todoitem_set.get(pk=pk)
...
def handle_post(request, data):
    ...
    item = request.user.todoitem_set.create(**data)
    return json_response(item.to_json(), status=201)
...
def handle_rest_call(request, pk=None, **kwargs):
    ...
    if request.user.is_anonymous():
        return error_json_response('Forbidden', status=403)
